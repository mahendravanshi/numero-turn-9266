...
let lurl = "https://numero-backend.vercel.app/mens";
let lData= [];
let length;
let noOfPages;
let limit = 20;
...
let url = `https://numero-backend.vercel.app/mens?_page=${count}&_limit=${limit}`
...
function filterPriceFun(){
    ...
    if(topSearch.value!=""){
        ...
        if(filterPrice.value=="asc"){
            let url2 = `https://numero-backend.vercel.app/mens?_sort=price&_order=asc&_page=${count}&_limit=${limit}`;
            fetchData(url2)

        }

        if(filterPrice.value=="desc"){
            let url3 = `https://numero-backend.vercel.app/mens?_sort=price&_order=desc&_page=${count}&_limit=${limit}`;
            fetchData(url3)
        }
    }
